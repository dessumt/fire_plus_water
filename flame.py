...
class FlameParticle:
    ...
    def __init__(self, x=SCREEN_WIDTH // 2, y=SCREEN_HEIGHT // 2, r=1):
        self.x = x
        self.y = y
        self.r = r
        self.original_r = r
        ...
        max_surf_size = self.r * self.alpha_layers * self.alpha_layers * self.alpha_glow
...
class Flame:
    def __init__(self, x=SCREEN_WIDTH // 2, y=SCREEN_HEIGHT // 2):
        ...
        for i in range(self.flame_intensity * 25):
            self.flame_particles.append(FlameParticle(self.x + random.randint(-1, 1), self.y, random.randint(1, 2)))

    def draw_flame(self):
        for i in self.flame_particles:
            if i.original_r <= 0:
                self.flame_particles.remove(i)
                self.flame_particles.append(FlameParticle(self.x + random.randint(-1, 1), self.y, random.randint(1, 4)))
                del i
                continue
            i.update()
            i.draw()
